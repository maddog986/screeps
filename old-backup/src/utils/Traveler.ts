...
export class Traveler {
  ...
  public static travelTo(
    ...
    if (!creep.memory._trav) {
      creep.memory._trav = { path: "" } as TravelData;
    }
  ...
  public static checkAvoid(): boolean {
    return false;
  }
  ...
  public static findTravelPath(
    ...
    const callback = (roomName: string): CostMatrix | boolean => {
      ...
      } else if (!options.allowHostile && roomName !== destRoomName && roomName !== originRoomName) {
        return false;
      }
  ...
  public static findRoute(
    ...
    const ret = Game.map.findRoute(origin, destination, {
      routeCallback: (roomName: string) => {
        ...
        if (!options.allowHostile && roomName !== destination && roomName !== origin) {
